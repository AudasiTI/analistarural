(function(undefined) {
	...
	if (typeof XLS === 'undefined') {
		console
				.log('xls.js is required. Get it from https://github.com/SheetJS/js-xls');
		return;
	}
	...
	XLSReader.utils = {
		'intializeFromFile' : function(obj, file, readCells, toJSON, handler) {
			...
			reader.onload = function(e) {
				...
				obj.sheets = XLSReader.utils.parseWorkbook(workbook, readCells,
						toJSON);
				handler(obj);
			}
		...
		'parseWorkbook' : function(workbook, readCells, toJSON) {
			...
			_.forEachRight(workbook.SheetNames, function(sheetName) {
				...
				sheets[sheetName] = XLSReader.utils
