...
(function() {
	...
	// Hook httpInterceptor factory into the $httpProvider interceptors so that
	// we can monitor XHR calls
	overlayApp.config([ '$httpProvider', httpProvider ]);
	function httpProvider($httpProvider) {
		$httpProvider.interceptors.push('ngplus.httpInterceptor');
	}

	// Directive that uses the httpInterceptor factory above to monitor XHR
	// calls
	...
	overlayApp.directive('ngplusOverlay', [ '$q', '$timeout', '$window',
			'ngplus.httpInterceptor', overlay ]);
	...
	function overlay($q, $timeout, $window, httpInterceptor) {
		...
		function link(scope, element, attrs) {
			...
			var delayIn = scope.ngplusOverlayDelayIn ? scope.ngplusOverlayDelayIn
					: defaults.overlayDelayIn;
			var delayOut = scope.ngplusOverlayDelayOut ? scope.ngplusOverlayDelayOut
					: defaults.overlayDelayOut;
			...
			function init() {
				...
				overlayContainer = document
						.getElementById('ngplus-overlay-container');
			}

			// Hook into httpInterceptor factory request/response/responseError
			// functions
			...
			function processRequest() {
				...
				if (queue.length == 1) {
					timerPromise = $timeout(function() {
						if (queue.length)
							showOverlay();
					}, delayIn); // Delay showing for 500 millis to avoid
									// flicker
				}
			}
			...
			function processResponse() {
				...
				if (queue.length == 0) {
					...
					// pause before
					// hiding the overlay. If another XHR request comes in then
					// the overlay
					// will stay visible which prevents a flicker
					timerPromiseHide = $timeout(function() {
						// Make sure queue is still 0 since a new XHR request
						// may have come in
			...
			var getComputedStyle = (function() {
				...
				if (document.defaultView
					...
					// Polyfill for getComputedStyle from:
					// https://gist.github.com/twolfson/5369885
					func = function(el, prop, getComputedStyle) {
						...
						// Query it
						// TODO: From CSS-Query notes, we might need (node,
						// null) for FF
						getComputedStyle(el) :
